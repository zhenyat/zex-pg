class Admin::CoinNicknamesController < Admin::BaseController
  ...
  def create
    ...
    if @coin_nickname.save
      flash[:success] = t('messages.created', model: @coin_nickname.class.model_name.human)
      redirect_to [:admin, @coin_nickname]
    else      
      render :new
    end
  end
  ...
  def update
    ...
    if @coin_nickname.update(coin_nickname_params)
      flash[:success] = t('messages.updated', model: @coin_nickname.class.model_name.human)
      redirect_to [:admin, @coin_nickname]
    else      
      render :edit
    end
  ...
  private
    ...
    def coin_nickname_params
      params.require(:coin_nickname).permit(:coin_id, :name)
    end
end
